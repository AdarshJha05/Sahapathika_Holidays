...
const insertionPoint = "} else if (p.slug === 'kerala-ayurveda-wellness-retreat') {";
const varanasiCode = `} else if (p.slug === 'varanasi-tour-packages-from-kerala') {
...
htmlFiles.forEach(file => {
  ...
  // Add d.mapUrl support
  const mapUrlRegex = /mapUrl: 'https:\/\/maps\.google\.com\/maps\?q=' \+ encodeURIComponent\(p\.regions \|\| p\.title\) \+ '&t=&z=6&ie=UTF8&iwloc=&output=embed',/g;
  const mapUrlReplacement = `mapUrl: d.mapUrl || ('https://maps.google.com/maps?q=' + encodeURIComponent(p.regions || p.title) + '&t=&z=6&ie=UTF8&iwloc=&output=embed'),`;
  if (content.match(mapUrlRegex)) {
    content = content.replace(mapUrlRegex, mapUrlReplacement);
    changed = true;
  }
