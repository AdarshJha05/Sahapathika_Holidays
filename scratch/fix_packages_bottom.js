...
const old = fs.readFileSync('scratch/old_packages.html', 'utf8');
...
let fixedBottom = oldBottom.replace(/v\.play\(\)\.catch\(function\(\)\{\}\);/g, "if(v.play) { var p = v.play(); if (p && p.catch) p.catch(function(){}); }");
fixedBottom = fixedBottom.replace(/n\.play\(\)\.catch\(function\(\)\{\}\);/g, "if(n.play) { var p = n.play(); if (p && p.catch) p.catch(function(){}); }");
